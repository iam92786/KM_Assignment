...
int m_close(struct inode *, struct file *);
...
ssize_t m_read(struct file *fptr, char __user *usr, size_t size, loff_t *off)
{
...
int m_open(struct inode *node, struct file *fptr)
{
    printk("device opened\n");
    return 0;
}

int m_close(struct inode *node, struct file *fptr)
{
